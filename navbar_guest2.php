<!-- THIS IS USED IN LOGIN/REGISTER AND RECOVER PASSWORD PAGE AS THE BOOTSTRAP CLASSES CLASH TIWH THET CUSTOME CSS CLASSES  -->

<head>
  <link rel="stylesheet" href="css/newnavbar.css">
  <title></title>
</head>

<body>
  <header>
    <input type="checkbox" id="hamburger-input" class="burger-shower" />
    <label id="hamburger-menu" for="hamburger-input" style='z-index:999'>
      <nav id="sidebar-menu" style="overflow-y: auto;">

        <h3>Menu </h3>
        <ul>
          <li><a href="homepage.php">Hompage</a></li>
          <li><a href="emergency.php">Emergency</a></li>
          <li><a href="self_checkup_form.php">Self Checkup</a></li>
          <li><a href="Knowledge.php">Knowledge</a></li>
          <li><a href="games.php">Games</a></li>
          <li><a href="about.php">About</a></li>
          <li><a href="contact.php">Contact Us</a></li>
          <li><a href="login.php">Login</a></li>
        </ul>
        <h4>Close</h4>

      </nav>
    </label>
    <div class="overlay"></div>

  </header>
</body>